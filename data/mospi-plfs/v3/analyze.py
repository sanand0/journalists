...
def setup_views(con: duckdb.DuckDBPyConnection) -> None:
    ...
    con.execute(
        ...
        SELECT
          ...
          MAX(CASE WHEN "Age" BETWEEN 0 AND 2 THEN 1 ELSE 0 END) AS has_child_0_2
        FROM base
        GROUP BY 1
        """
    )
    ...
    con.execute(
        ...
        SELECT
          ...
          CASE WHEN "Subsidiary Work Engagement" = 'yes' THEN 1 ELSE 0 END AS subsidiary
        FROM base
        WHERE "Sex" = 'female'
    ...
    con.execute(
        """
        CREATE OR REPLACE VIEW person_day_casual AS
        SELECT
          person_id,
          sex,
          sector,
          ...
          day_no,
          MAX(w) AS w,
          SUM(hours) AS day_hours,
          SUM(wage) AS day_wage,
          SUM(wage) / SUM(hours) AS hourly
        FROM diary_all
        WHERE status IN (
...
def export_child_proxy_models(con: duckdb.DuckDBPyConnection) -> None:
    """Estimate the child-in-household association for married graduate women."""
    ...
    frame = con.execute(
        ...
        SELECT
          hh.has_child_0_5,
          hh.has_child_0_2,
          CASE WHEN b."CWS Status" = 'attended domestic duties only' THEN 1 ELSE 0 END AS domestic_only,
          CASE WHEN b."CWS Status" = 'worked as regular salaried/wage employee' THEN 1 ELSE 0 END AS regular_salaried,
          b."Sector" AS sector,
          b."Household Social Group" AS social_group,
          b."Household Type" AS household_type,
          b."Household Religion" AS religion,
          b."State/UT" AS state,
          b."Person Subsample Multiplier" / ? AS model_weight
        FROM base b
        JOIN households hh USING (hhid)
        WHERE b."Sex" = 'female'
          AND b."Age" BETWEEN 25 AND 34
          AND b."Marital Status" = 'currently married'
          AND b.edu_band = 'graduate+'
        """,
        [MODEL_WEIGHT_SCALE],
    ).fetch_df()

    rows: list[dict[str, object]] = []
    for child_col in ("has_child_0_5", "has_child_0_2"):
        for outcome in ("domestic_only", "regular_salaried"):
            model = smf.wls(
                (
                    f"{outcome} ~ {child_col} + C(sector)"
                    " + C(social_group) + C(household_type)"
                    " + C(religion) + C(state)"
                ),
                data=frame,
            ...
            rows.append(
                {
                    "child_proxy": child_col,
                    "outcome": outcome,
                    "child_effect": round(model.params[child_col], 4),
                    "child_effect_points": round(100 * model.params[child_col], 2),
                    "std_error": round(model.bse[child_col], 4),
                    "std_error_points": round(100 * model.bse[child_col], 2),
                    "sample_n": len(frame),
                    "weighted_total_m": round(frame["model_weight"].sum() / 1000, 3),
                    "controls": (
                        "state + sector + social group"
                        " + household type + religion"
                    ),
                }
            )

    write_rows(OUTPUT_DIR / "child_proxy_married_graduate_model.csv", rows)
...
def main() -> None:
    ...
    export_csv(
        ...
        SELECT
          COUNT(*) AS household_keys,
          COUNT(*) FILTER (
            WHERE people_rows = min_household_size
              AND min_household_size = max_household_size
          ) AS exact_size_match_keys,
          MAX(people_rows) AS max_people_rows,
          COUNT(*) FILTER (WHERE people_rows = 1) AS single_person_households
        FROM hh
        """,
    )
    ...
    export_csv(
        con,
        "hidden_work_national_benchmark.csv",
        f"""
        SELECT
          COUNT(*) AS sample_n,
          ROUND(SUM(w) / {WEIGHT_TO_MILLIONS}, 3) AS total_m,
          ROUND(100.0 * SUM(w) FILTER (WHERE worked_week = 1) / SUM(w), 1) AS worked_week_pct,
          ROUND(100.0 * SUM(w) FILTER (WHERE subsidiary = 1) / SUM(w), 1) AS subsidiary_pct,
          ROUND(
            100.0 * SUM(w) FILTER (WHERE worked_week = 1 AND subsidiary = 1) / SUM(w),
            1
          ) AS worked_week_and_subsidiary_pct
    ...
    export_csv(
        con,
        "hidden_work_weekly_work_composition.csv",
        f"""
        SELECT
          cws_status,
          COUNT(*) AS sample_n,
          ROUND(SUM(w) / {WEIGHT_TO_MILLIONS}, 3) AS weighted_m,
          ROUND(100.0 * SUM(w) / SUM(SUM(w)) OVER (), 1) AS share_hidden_workers_pct
        FROM hidden_domestic
        WHERE worked_week = 1
        GROUP BY 1
        ORDER BY weighted_m DESC, sample_n DESC
        """,
    )

    export_csv(
        con,
        "hidden_work_state_mass.csv",
        f"""
        WITH agg AS (
          SELECT
            state,
            COUNT(*) AS sample_n,
            SUM(w) AS total_w,
            SUM(w) FILTER (WHERE worked_week = 1) AS worked_w,
            SUM(w) FILTER (WHERE subsidiary = 1) AS subsidiary_w
          FROM hidden_domestic
          GROUP BY 1
        )
        SELECT
          ...
          ROUND(100.0 * worked_w / total_w, 1) AS worked_week_pct,
          ROUND(100.0 * subsidiary_w / total_w, 1) AS subsidiary_pct,
          ROUND(100.0 * worked_w / SUM(worked_w) OVER (), 1) AS share_all_hidden_workers_pct
        FROM agg
        WHERE sample_n >= 1000
        ORDER BY worked_week_pct DESC, sample_n DESC
        """,
    )

    export_csv(
        con,
        "hidden_work_concentration_benchmarks.csv",
        f"""
        SELECT
          'rural' AS subgroup,
          ROUND(100.0 * SUM(w) FILTER (WHERE sector = 'rural') / SUM(w), 1) AS share_domestic_only_pct,
          ROUND(
            100.0 * SUM(w) FILTER (WHERE sector = 'rural' AND worked_week = 1)
            / SUM(w) FILTER (WHERE worked_week = 1),
            1
          ) AS share_hidden_workers_pct
        FROM hidden_domestic
        UNION ALL
        SELECT
          'self-employed in agriculture households' AS subgroup,
          ROUND(
            100.0
            * SUM(w) FILTER (WHERE household_type = 'self-employed in agriculture')
            / SUM(w),
            1
          ) AS share_domestic_only_pct,
          ROUND(
            100.0
            * SUM(w)
              FILTER (
                WHERE household_type = 'self-employed in agriculture'
                  AND worked_week = 1
              )
            / SUM(w) FILTER (WHERE worked_week = 1),
            1
        ...
        UNION ALL
        SELECT
          'rural age 25-59 up to primary schooling' AS subgroup,
          ROUND(
            100.0
            * SUM(w)
              FILTER (
                WHERE sector = 'rural'
                  AND age BETWEEN 25 AND 59
                  AND education IN ('not literate', 'literate: below primary', 'primary')
              )
            / SUM(w),
            1
          ) AS share_domestic_only_pct,
    ...
    export_csv(
        con,
        "hidden_work_strong_subgroup_state.csv",
        f"""
        WITH strong AS (
          SELECT *
          FROM hidden_domestic
        ...
        SELECT
          state,
          COUNT(*) AS sample_n,
          COUNT(*) FILTER (WHERE worked_week = 1) AS worked_week_n,
          ROUND(SUM(w) / {WEIGHT_TO_MILLIONS}, 3) AS total_m,
          ROUND(100.0 * SUM(w) FILTER (WHERE worked_week = 1) / SUM(w), 1) AS worked_week_pct,
          ROUND(100.0 * SUM(w) FILTER (WHERE subsidiary = 1) / SUM(w), 1) AS subsidiary_pct
        FROM strong
    ...
    export_csv(
        con,
        "child_proxy_women_25_34_under5.csv",
        f"""
        WITH base AS (
          SELECT
            CONCAT_WS(
        ...
        SELECT
          ...
          ROUND(
            ...
            * SUM("Person Subsample Multiplier")
              FILTER (WHERE "CWS Status" = 'attended domestic duties only')
            / SUM("Person Subsample Multiplier"),
            1
          ...
          ROUND(
            100.0
            * SUM("Person Subsample Multiplier")
              FILTER (WHERE "CWS Status" = 'worked as regular salaried/wage employee')
            / SUM("Person Subsample Multiplier"),
            1
        ...
        GROUP BY 1, 2, 3
        HAVING sample_n >= 100
        ORDER BY edu_band, marital_status, hh.has_child_0_5 DESC
        """,
    )

    export_csv(
        con,
        "child_proxy_women_25_34_under2.csv",
        f"""
        WITH base AS (
          SELECT
            CONCAT_WS(
        ...
        GROUP BY 1, 2, 3
        HAVING sample_n >= 100
        ORDER BY edu_band, marital_status, hh.has_child_0_2 DESC
        """,
    )

    export_csv(
        con,
        "child_proxy_married_graduate_state_under5.csv",
        f"""
        WITH base AS (
          SELECT
            CONCAT_WS(
        ...
        cells AS (
          ...
          WHERE "Sex" = 'female'
            AND "Age" BETWEEN 25 AND 34
            AND "Marital Status" = 'currently married'
            AND edu_band = 'graduate+'
          GROUP BY 1, 2
        )
        SELECT
          child.state,
          child.sample_n AS child_sample_n,
          no_child.sample_n AS no_child_sample_n,
          child.domestic_only_pct AS domestic_child_pct,
          no_child.domestic_only_pct AS domestic_no_child_pct,
          ROUND(child.domestic_only_pct - no_child.domestic_only_pct, 1) AS domestic_gap_pts,
          child.regular_salaried_pct AS regular_child_pct,
          no_child.regular_salaried_pct AS regular_no_child_pct,
          ROUND(child.regular_salaried_pct - no_child.regular_salaried_pct, 1) AS regular_gap_pts
        FROM cells child
        JOIN cells no_child USING (state)
        WHERE child.has_child_0_5 = 1
          AND no_child.has_child_0_5 = 0
          AND child.sample_n >= 100
          AND no_child.sample_n >= 50
        ORDER BY domestic_gap_pts DESC, child_sample_n DESC
        """,
    )

    export_csv(
        con,
        "child_proxy_married_graduate_sex_compare_under2.csv",
        f"""
        WITH base AS (
          SELECT
            CONCAT_WS(
        ...
        SELECT
          ...
          ROUND(
            100.0
            * SUM("Person Subsample Multiplier")
              FILTER (WHERE "CWS Status" LIKE 'worked%')
            / SUM("Person Subsample Multiplier"),
            1
          ) AS worked_any_pct,
          ROUND(
        ...
        FROM base
        JOIN households hh USING (hhid)
        WHERE "Marital Status" = 'currently married'
          AND "Age" BETWEEN 25 AND 34
          AND edu_band = 'graduate+'
          AND "Sex" IN ('female', 'male')
        GROUP BY 1, 2
        ORDER BY sex, hh.has_child_0_2 DESC
        """,
    )
    ...
    export_csv(
        con,
        "graduate_worker_occupation_buckets.csv",
        f"""
        WITH base AS (
          SELECT
            ...
            "Person Subsample Multiplier" AS w
          FROM plfs
          WHERE edu_band = 'graduate+'
            AND "Principal Occupation" IS NOT NULL
        ),
        grads AS (
          SELECT 'all_graduates' AS age_group, occupation, w
          FROM base
          UNION ALL
          SELECT 'age_21_34' AS age_group, occupation, w
          FROM base
        ...
        buckets AS (
          SELECT
            ...
            CASE
              ...
              ) THEN 'farm_and_animal_work'
              WHEN occupation IN (
                'Shop Salespersons',
                'Street and Market Salespersons',
                'Other Sales Workers'
              ) THEN 'sales_work'
              WHEN occupation LIKE '%Teachers%' THEN 'teaching'
              WHEN occupation IN (
                'Software and Application Developers and Analysts',
                'Engineering Professionals (Excluding Electrotechnology)',
                'Engineering Professionals'
              ) THEN 'software_and_engineering'
              WHEN occupation IN (
                'Medical Doctors',
        ...
        SELECT
          ...
          occupation_bucket,
          ROUND(SUM(w) / {WEIGHT_TO_MILLIONS}, 3) AS weighted_m,
          ROUND(100.0 * SUM(w) / SUM(SUM(w)) OVER (PARTITION BY age_group), 1) AS share_pct
        FROM buckets
        GROUP BY 1, 2
        ORDER BY age_group, weighted_m DESC
        """,
    )

    export_csv(
        con,
        "graduate_worker_top_occupations.csv",
        f"""
        SELECT
          "Principal Occupation" AS occupation,
          COUNT(*) AS sample_n,
          ...
          ROUND(
            100.0 * SUM("Person Subsample Multiplier")
            / SUM(SUM("Person Subsample Multiplier")) OVER (),
            1
          ) AS share_pct
        FROM plfs
        WHERE edu_band = 'graduate+'
    ...
    export_csv(
        con,
        "graduate_regular_selected_occupations.csv",
        f"""
        SELECT
          "Principal Occupation" AS occupation,
          COUNT(*) AS sample_n,
    ...
    export_csv(
        con,
        "nonformal_regular_top_occupations.csv",
        f"""
        SELECT
          "Principal Occupation" AS occupation,
          COUNT(*) AS sample_n,
          ROUND(SUM("Person Subsample Multiplier") / {WEIGHT_TO_MILLIONS}, 3) AS nonformal_regular_m,
          ROUND(
            100.0 * SUM("Person Subsample Multiplier")
            / SUM(SUM("Person Subsample Multiplier")) OVER (),
            1
        ...
        FROM plfs
        WHERE is_regular_salaried = 1
          AND is_strict_formal = 0
          AND "Principal Occupation" IS NOT NULL
        GROUP BY 1
        HAVING sample_n >= 80
        ORDER BY nonformal_regular_m DESC
        LIMIT 25
        """,
    )
    ...
    export_csv(
        con,
        "technical_education_search_benchmark.csv",
        f"""
        WITH base AS (
          SELECT
            ...
            "Person Subsample Multiplier" AS w
          FROM plfs
          WHERE "Technical Education Level" IS NOT NULL
        ...
        SELECT
          ...
          COUNT(*) AS sample_n,
          ROUND(SUM(w) / {WEIGHT_TO_MILLIONS}, 3) AS total_m,
          ROUND(100.0 * SUM(w) FILTER (WHERE cws_status = 'sought work') / SUM(w), 1) AS seeker_pct,
          ROUND(
            100.0
            * SUM(w) FILTER (WHERE cws_status = 'worked as regular salaried/wage employee')
            / SUM(w),
            1
          ) AS regular_salaried_pct
        FROM base
        GROUP BY 1, 2, 3
        HAVING sample_n >= 150
        ORDER BY seeker_pct DESC, regular_salaried_pct ASC
        """,
    )

    export_csv(
        con,
        "engineering_education_benchmark.csv",
        f"""
        WITH base AS (
          SELECT
            "Technical Education Level" AS technical_education,
            ...
            "Person Subsample Multiplier" AS w
          FROM plfs
          WHERE "Technical Education Level" IN (
        ...
        GROUP BY 1, 2, 3, 4
        HAVING sample_n >= 75
        ORDER BY technical_education, seeker_pct DESC, regular_salaried_pct ASC
        """,
    )

    export_csv(
        con,
        "technical_education_low_skill_jobs.csv",
        f"""
        WITH base AS (
          SELECT
            "Technical Education Level" AS technical_education,
        ...
        SELECT
          ...
          ROUND(
            ...
            * SUM(
                w * CASE
                  WHEN occupation LIKE '%Labourers%'
                    OR occupation IN (
            ...
            / SUM(w),
            1
          ) AS low_skill_plus_sales_pct
        FROM base
        GROUP BY 1
        HAVING sample_n >= 150
        ORDER BY low_skill_plus_sales_pct DESC, total_m DESC
        """,
    )
    ...
    export_csv(
        con,
        "casual_person_day_rural.csv",
        f"""
        SELECT
          sex,
          CASE
            WHEN status = 'worked as casual wage labour: in other types of work'
              THEN 'other_casual'
            WHEN status = 'worked as casual wage labour in MGNREG works'
              THEN 'mgnreg'
            ELSE 'other_public_works'
          END AS status_group,
          COUNT(*) AS person_day_rows,
          ROUND(AVG(day_hours), 2) AS avg_day_hours,
          ROUND(AVG(day_wage), 2) AS avg_day_wage,
          ROUND(SUM(w * day_wage) / SUM(w * day_hours), 2) AS weighted_ratio_hourly,
          ROUND(quantile_cont(hourly, 0.5), 2) AS median_person_day_hourly
        FROM person_day_casual
        WHERE sector = 'rural'
        GROUP BY 1, 2
        ORDER BY sex, status_group
        """,
    )

    export_csv(
        con,
        "casual_person_day_rural_6_8h.csv",
        f"""
        SELECT
          sex,
          CASE
    ...
    export_csv(
        con,
        "casual_mgnreg_state_compare.csv",
        f"""
        WITH rates AS (
          SELECT
            sex,
            ...
            status,
            COUNT(*) AS person_day_rows,
            SUM(w * day_wage) / SUM(w * day_hours) AS weighted_ratio_hourly
          FROM person_day_casual
          WHERE sector = 'rural'
            AND status IN (
              'worked as casual wage labour: in other types of work',
        ...
        SELECT
          other.sex,
          other.state,
          other.person_day_rows AS other_rows,
          mgnreg.person_day_rows AS mgnreg_rows,
          ROUND(other.weighted_ratio_hourly, 2) AS other_hourly,
          ROUND(mgnreg.weighted_ratio_hourly, 2) AS mgnreg_hourly,
          ROUND(mgnreg.weighted_ratio_hourly - other.weighted_ratio_hourly, 2) AS mgnreg_minus_other
        FROM rates other
        JOIN rates mgnreg USING (sex, state)
        WHERE other.status = 'worked as casual wage labour: in other types of work'
          AND mgnreg.status = 'worked as casual wage labour in MGNREG works'
          AND other.person_day_rows >= 100
          AND mgnreg.person_day_rows >= 100
        ORDER BY sex, mgnreg_minus_other DESC, other_rows DESC
        """,
    )
