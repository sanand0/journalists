...
"""Generate reproducible v3 insight tables from the local UN data slices."""
...
def require_metadata() -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            f"Missing metadata files: {missing_list}. Run `uv run v3/build_metadata.py` first."
        )
...
def normalize_binary_status(values: pd.Series) -> pd.Series:
    """Convert a numeric status series into nullable 0/1 flags."""

    normalized = pd.Series(pd.NA, index=values.index, dtype="Int64")
    mask = values.notna()
    normalized.loc[mask] = (values.loc[mask] > 0).astype(int)
    return normalized
...
def build_candidate_indicator_profile(sdg: pd.DataFrame, country_sdg: pd.DataFrame, codes: pd.DataFrame) -> None:
    """Summarize the main series screened for v3."""
...
def build_seed_outputs(
    ...
    """Summarize crop-gene transfer activity and treaty-vs-framework gaps."""

    area_labels = label_map(codes, "CL_AREA")

    smta = latest_series(
        country_sdg,
        series="ER_CBD_SMTA",
        filters={},
        value_name="smta_count",
    )
    ranking = with_country_names(smta, country_map).assign(smta_count=lambda df: df["smta_count"].round(0).astype(int))
    ranking = ranking.sort_values(["smta_count", "area_name"], ascending=[False, True]).reset_index(drop=True)
    ranking["rank_high_to_low"] = ranking.index + 1
    write_csv(ranking, "seed_smta_country_rank.csv")

    write_csv(
        ranking.loc[ranking["area_name"].isin(INDIA_PEERS)].copy(),
        "seed_smta_india_peers.csv",
    )

    keep_columns = ["REF_AREA", "TIME_PERIOD", "time_period_sort", "obs_value_num"]
    treaty = latest_per_area(
        country_sdg.loc[country_sdg["SERIES"] == "ER_CBD_PTYPGRFA", keep_columns].copy(),
        sort_columns=["time_period_sort"],
        ascending=[False],
    ).rename(columns={"obs_value_num": "treaty_raw"})
    framework = latest_per_area(
        country_sdg.loc[country_sdg["SERIES"] == "ER_CBD_ORSPGRFA", keep_columns].copy(),
        sort_columns=["time_period_sort"],
        ascending=[False],
    ).rename(columns={"obs_value_num": "framework_raw"})

    status = (
        country_map.merge(treaty[["REF_AREA", "treaty_raw"]], left_on="area_code", right_on="REF_AREA", how="left")
        .drop(columns=["REF_AREA"])
        .merge(framework[["REF_AREA", "framework_raw"]], left_on="area_code", right_on="REF_AREA", how="left")
        .drop(columns=["REF_AREA"])
        .sort_values("area_name")
        .reset_index(drop=True)
    )
    status = status.loc[status[["treaty_raw", "framework_raw"]].notna().any(axis=1)].copy()
    status["treaty_member"] = normalize_binary_status(status["treaty_raw"])
    status["framework_in_place"] = normalize_binary_status(status["framework_raw"])
    status["treaty_raw"] = status["treaty_raw"].round(2)
    status["framework_raw"] = status["framework_raw"].round(2)
    write_csv(status, "seed_treaty_status.csv")

    full_smta = latest_series(
        sdg,
        series="ER_CBD_SMTA",
        filters={},
        value_name="smta_count",
    ).assign(area_name=lambda df: df["REF_AREA"].map(area_labels).fillna(""))
    southern_asia_row = full_smta.loc[full_smta["area_name"] == "Southern Asia"]

    status_complete = status.dropna(subset=["treaty_member", "framework_in_place"]).copy()
    india_row = ranking.loc[ranking["area_name"] == "India"].iloc[0]
    china_row = ranking.loc[ranking["area_name"] == "China"].iloc[0]
    signed_but_no_framework = int(
        ((status_complete["treaty_member"] == 1) & (status_complete["framework_in_place"] == 0)).sum()
    )
    ...
    summary = pd.DataFrame(
        [
            {
                "reporting_areas": len(ranking),
                "median_smta_count": round(float(ranking["smta_count"].median()), 2),
                "top_area": ranking.iloc[0]["area_name"],
                "top_smta_count": int(ranking.iloc[0]["smta_count"]),
                "india_smta_count": int(india_row["smta_count"]),
                "india_rank_high_to_low": int(india_row["rank_high_to_low"]),
                "china_smta_count": int(china_row["smta_count"]),
                "india_vs_china_ratio": round(float(india_row["smta_count"]) / float(china_row["smta_count"]), 2),
                "southern_asia_smta_count": float(southern_asia_row["smta_count"].iloc[0]) if not southern_asia_row.empty else np.nan,
                "india_share_of_southern_asia_pct": round(
                    float(india_row["smta_count"]) / float(southern_asia_row["smta_count"].iloc[0]) * 100,
                    2,
                )
                if not southern_asia_row.empty
                else np.nan,
                "treaty_reporting_areas": len(status_complete),
                "treaty_members": int(status_complete["treaty_member"].sum()),
                "framework_members": int(status_complete["framework_in_place"].sum()),
                "signed_but_no_framework": signed_but_no_framework,
                "signed_but_no_framework_share_of_members_pct": round(
                    signed_but_no_framework / float(status_complete["treaty_member"].sum()) * 100,
                    2,
                ),
            }
        ]
    )
    write_csv(summary, "seed_treaty_summary.csv")


def build_livestock_outputs(country_sdg: pd.DataFrame, country_map: pd.DataFrame) -> None:
    """Summarize how much livestock diversity is backed up for recovery."""

    local_breeds = latest_series(
        country_sdg,
        series="ER_GRF_ANIMKPT",
        filters={},
        value_name="local_breeds",
    )
    local_backup = latest_series(
        country_sdg,
        series="ER_GRF_ANIMRCNTN",
        filters={},
        value_name="backed_up_local_breeds",
    )
    transboundary_backup = latest_series(
        country_sdg,
        series="ER_GRF_ANIMRCNTN_TRB",
        filters={},
        value_name="backed_up_transboundary_breeds",
    )
    at_risk = latest_series(
        country_sdg,
        series="ER_RSK_LBREDS",
        filters={},
        value_name="at_risk_share_pct",
    )

    livestock = (
        country_map.merge(local_breeds[["REF_AREA", "local_breeds"]], left_on="area_code", right_on="REF_AREA", how="left")
        .drop(columns=["REF_AREA"])
        .merge(local_backup[["REF_AREA", "backed_up_local_breeds"]], left_on="area_code", right_on="REF_AREA", how="left")
        .drop(columns=["REF_AREA"])
        .merge(
            transboundary_backup[["REF_AREA", "backed_up_transboundary_breeds"]],
            left_on="area_code",
            right_on="REF_AREA",
            how="left",
        )
        .drop(columns=["REF_AREA"])
        .merge(at_risk[["REF_AREA", "at_risk_share_pct"]], left_on="area_code", right_on="REF_AREA", how="left")
        .drop(columns=["REF_AREA"])
        .sort_values("area_name")
        .reset_index(drop=True)
    )
    livestock = livestock.loc[
        livestock[
            [
                "local_breeds",
                "backed_up_local_breeds",
                "backed_up_transboundary_breeds",
                "at_risk_share_pct",
            ]
        ].notna().any(axis=1)
    ].copy()
    livestock["backed_up_local_share_pct"] = (
        livestock["backed_up_local_breeds"] / livestock["local_breeds"] * 100
    )

    backup_rank = (
        livestock.loc[livestock["backed_up_local_breeds"].notna(), ["area_name", "backed_up_local_breeds"]]
        .sort_values(["backed_up_local_breeds", "area_name"], ascending=[False, True])
        .reset_index(drop=True)
    )
    backup_rank["backup_rank_high_to_low"] = backup_rank.index + 1
    livestock = livestock.merge(backup_rank, on=["area_name", "backed_up_local_breeds"], how="left")

    for column in [
        "local_breeds",
        "backed_up_local_breeds",
        "backed_up_transboundary_breeds",
        "backed_up_local_share_pct",
        "at_risk_share_pct",
    ]:
        livestock[column] = livestock[column].round(2)

    livestock_rank = livestock.sort_values(
        ["backup_rank_high_to_low", "area_name"],
        ascending=[True, True],
        na_position="last",
    ).reset_index(drop=True)
    write_csv(livestock_rank, "livestock_genetic_backup_country_rank.csv")
    write_csv(
        livestock_rank.loc[livestock_rank["area_name"].isin(INDIA_PEERS)].copy(),
        "livestock_genetic_backup_india_peers.csv",
    )

    risk_rank = (
        livestock.loc[livestock["at_risk_share_pct"].notna(), ["area_name", "at_risk_share_pct", "local_breeds", "backed_up_local_breeds"]]
        .sort_values(["at_risk_share_pct", "area_name"], ascending=[False, True])
        .reset_index(drop=True)
    )
    risk_rank["risk_rank_high_to_low"] = risk_rank.index + 1
    write_csv(risk_rank, "livestock_breed_risk_country_rank.csv")

    backup_complete = livestock.dropna(subset=["local_breeds", "backed_up_local_breeds"]).copy()
    risk_complete = livestock.dropna(subset=["at_risk_share_pct"]).copy()
    joint_risk = livestock.dropna(subset=["backed_up_local_breeds", "at_risk_share_pct"]).copy()
    india_row = livestock.loc[livestock["area_name"] == "India"].iloc[0]
    zero_backup_areas = int((backup_complete["backed_up_local_breeds"] == 0).sum())

    summary = pd.DataFrame(
        [
            {
                "reporting_areas_with_backup_data": len(backup_complete),
                "median_local_breeds": round(float(backup_complete["local_breeds"].median()), 2),
                "median_backed_up_local_breeds": round(float(backup_complete["backed_up_local_breeds"].median()), 2),
                "median_backed_up_local_share_pct": round(float(backup_complete["backed_up_local_share_pct"].median()), 2),
                "zero_local_backup_areas": zero_backup_areas,
                "zero_local_backup_share_pct": round(zero_backup_areas / len(backup_complete) * 100, 2),
                "areas_with_risk_data": len(risk_complete),
                "median_at_risk_share_pct": round(float(risk_complete["at_risk_share_pct"].median()), 2),
                "areas_half_or_more_at_risk": int((risk_complete["at_risk_share_pct"] >= 50).sum()),
                "zero_backup_and_half_at_risk": int(
                    ((joint_risk["backed_up_local_breeds"] == 0) & (joint_risk["at_risk_share_pct"] >= 50)).sum()
                ),
                "top_backup_area": backup_rank.iloc[0]["area_name"],
                "top_backed_up_local_breeds": float(backup_rank.iloc[0]["backed_up_local_breeds"]),
                "india_local_breeds": float(india_row["local_breeds"]),
                "india_backed_up_local_breeds": float(india_row["backed_up_local_breeds"]),
                "india_backed_up_local_share_pct": float(india_row["backed_up_local_share_pct"]),
                "india_at_risk_share_pct": float(india_row["at_risk_share_pct"]),
            }
        ]
    )
    write_csv(summary, "livestock_genetic_backup_summary.csv")


def build_water_participation_outputs(country_sdg: pd.DataFrame, country_map: pd.DataFrame) -> None:
    """Compare water-participation rules on paper with participation in practice."""

    procedures = latest_series(
        country_sdg,
        series="ER_WAT_PRDU",
        filters={},
        value_name="procedure_score",
    )
    participation = latest_series(
        country_sdg,
        series="ER_WAT_PART",
        filters={},
        value_name="participation_score",
    )

    procedure_labels = {
        10.0: "Clearly defined",
        5.0: "Not clearly defined",
        0.0: "No or not reported",
    }
    ...
    water = (
        country_map.merge(procedures[["REF_AREA", "procedure_score"]], left_on="area_code", right_on="REF_AREA", how="inner")
        .drop(columns=["REF_AREA"])
        .merge(participation[["REF_AREA", "participation_score"]], left_on="area_code", right_on="REF_AREA", how="inner")
        .drop(columns=["REF_AREA"])
        .sort_values("area_name")
        .reset_index(drop=True)
    )
    water = water.assign(
        procedure_label=lambda df: df["procedure_score"].map(procedure_labels).fillna("Unknown"),
        participation_label=lambda df: df["participation_score"].map(participation_labels).fillna("Unknown"),
        clear_procedures=lambda df: df["procedure_score"] == 10,
        high_participation=lambda df: df["participation_score"] == 3,
        clear_but_low_participation=lambda df: (df["procedure_score"] == 10) & (df["participation_score"] <= 1),
    )
    write_csv(water, "water_participation_country_scores.csv")

    write_csv(
        water.loc[water["clear_but_low_participation"]].copy(),
        "water_participation_clear_but_low_cases.csv",
    )
    write_csv(
        water.groupby(["participation_score", "participation_label"], as_index=False)
        .agg(areas=("area_name", "size"))
        .sort_values("participation_score", ascending=False)
        .reset_index(drop=True),
        "water_participation_distribution.csv",
    )
    write_csv(
        water.loc[water["area_name"].isin(INDIA_PEERS)].copy(),
        "water_participation_india_peers.csv",
    )

    clear_procedures = int(water["clear_procedures"].sum())
    high_participation = int(water["high_participation"].sum())
    clear_but_low = int(water["clear_but_low_participation"].sum())
    clear_and_high = int((water["clear_procedures"] & water["high_participation"]).sum())
    summary = pd.DataFrame(
        [
            {
                "reporting_areas": len(water),
                "areas_with_clearly_defined_procedures": clear_procedures,
                "share_with_clearly_defined_procedures_pct": round(clear_procedures / len(water) * 100, 2),
                "areas_with_high_participation": high_participation,
                "share_with_high_participation_pct": round(high_participation / len(water) * 100, 2),
                "areas_clear_but_low_participation": clear_but_low,
                "share_clear_but_low_participation_pct": round(clear_but_low / len(water) * 100, 2),
                "areas_clear_and_high_participation": clear_and_high,
                "share_clear_and_high_participation_pct": round(clear_and_high / len(water) * 100, 2),
            }
        ]
    )
    write_csv(summary, "water_participation_summary.csv")
...
def main() -> None:
    """Run the full v3 analysis pipeline."""
    ...
    build_seed_outputs(sdg, country_sdg, codes, country_map)
    build_livestock_outputs(country_sdg, country_map)
    build_water_participation_outputs(country_sdg, country_map)
